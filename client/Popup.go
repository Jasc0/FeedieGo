...
func initialListPopupModel(fc FeedieConfig, action func(FeedieConfig, []string) error, srcFunc func(FeedieConfig, string) []popUpListItem, multi bool, prev tea.Model, prompt string, values []string, end func(string) tea.Cmd) tea.Model {
	m := popUpModel{
		prevModel:       prev,
		display:         popupList,
		list:            list.New([]list.Item{}, FeediePopUpDelegate{multi: multi}, defaultW, defaultH),
	...
	var popupItems []popUpListItem
	if len(values) == 1 {
		popupItems = srcFunc(fc, values[0])
	} else {
		popupItems = srcFunc(fc, "")
	}
	items := make([]list.Item, len(popupItems))
	for i, p := range popupItems {
		items[i] = p
	}
		...
		case tea.KeyMsg:
			...
			if in(k, m.config.Keys["open"]) {
				if m.listMultiSelect {
					...
					sel, ok := m.list.SelectedItem().(popUpListItem)
					if ok {
...
func (d FeediePopUpDelegate) Render(w io.Writer, m list.Model, index int, listItem list.Item) {
	...
	if ok {
		...
		if d.multi {
			fmt.Fprintf(w, "%s%s%s", bar, selMark, title)
		} else {
